...
window.KIT_CATALOG = {
  ...
  "components": [
    ...
    },
    {
      ...
      "what": "Redacts sensitive data — emails, names, API keys, order data — before a screenshot is published, via Gaussian blur of the underlying pixels plus a 92% neutral-300 fill that keeps the result flat, even, and visible against a light background. No badge/icon.",
      ...
      "gotchas": [
        "Real backdrop blur strong enough that no text is recoverable, not a solid colour box — blurring in place keeps the surrounding UI’s shape and context intact.",
        "It is a *backdrop* filter, so it only obscures what is painted behind it: the box prop must overlap the pixels being redacted. Placed beside them it renders as an empty rounded rectangle — a silent no-op that still looks deliberate.",
        "Needs the screenshot inside its own backdrop root. An ancestor with filter, opacity below 1, or its own backdrop-filter starts a new one and cuts the screenshot out of what this can see — keep it a direct child of the positioned frame.",
        "Carries a 92% --color-neutral-300 fill alongside the blur — a deviation from component-privacy-blur.md's 'no fill of its own' line, made on direct instruction after backdrop-filter blur alone rendered a left-to-right gradient inside the box, confirmed present at every opacity down to 0% and every radius tested (8-30px) — intrinsic to backdrop-filter itself, not a symptom of the box overlapping blank background. A flat, non-backdrop fill hides that gradient in direct proportion to its own opacity; white (--color-neutral-0) needed ~97% to fully mask it, but at that point it's indistinguishable from the light card/page it usually sits on, i.e. the redaction becomes invisible rather than deliberate-looking. neutral-300 masks the same gradient at the same opacity while staying visibly a distinct grey patch. Consequence: no longer identical on any background — this light a fill over a genuinely dark screenshot reads as a pale patch. The spec doc hasn't been reconciled with this; treat it as open.",
        "Undersized regions still blur unevenly beneath the fill: an 18px radius needs real room to diffuse into. Redact the whole field row, not the text's exact cap height — the fill hides the smear, it doesn't replace giving the blur room.",
        "component-privacy-blur.md also documents a required marking badge (lock/eye-slash icon) so a blurred region reads as deliberate redaction, not a rendering bug — dropped from this implementation on direct instruction. Same open-spec caveat as the fill above.",
        "No longer light/dark-agnostic now that it carries a fill — see the fill gotcha above. There is still no on-dark class; the fill's tone doesn't currently adapt to ground."
      ]
    }
  ]
};
